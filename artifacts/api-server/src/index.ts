...
const BUSINESS_DEFAULT_SOURCES = [
...
async function seedSourcesIfEmpty(): Promise<void> {
  try {
    ...
    const defaultSources = isCryptoProfile() ? PANKOFF_CRYPTO_DEFAULT_SOURCES : BUSINESS_DEFAULT_SOURCES;
    await db.insert(sourcesTable).values(defaultSources);
    logger.info(
      { count: defaultSources.length, profile: isCryptoProfile() ? "crypto" : "business" },
      "Seeded default sources",
    );
  } catch (err) {
