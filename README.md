...
The bot reads Telegram channels using a **user account session** (not just the bot token). You need to generate a `TELEGRAM_STRING_SESSION` once and store it as an environment variable.
...
> **Security:** The session string grants full access to your Telegram account. Never commit it to version control, share it, or store it anywhere public. The `.gitignore` already excludes `.env` and `*.session` files.
...
| `TELEGRAM_API_ID` | ✅ | From my.telegram.org/apps — used by Telethon to read channels |
| `TELEGRAM_API_HASH` | ✅ | From my.telegram.org/apps — used by Telethon to read channels |
| `TELEGRAM_STRING_SESSION` | ✅ | Generated by `generate_session.py` — user account session for reading channels |
